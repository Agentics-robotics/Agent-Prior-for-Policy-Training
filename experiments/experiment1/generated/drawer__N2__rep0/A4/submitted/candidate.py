...
class HorizontalErrorResidualDesign(CandidateDesign):
    def __init__(self, common_spec, config):
        ...
        self.error_gain = float(config["horizontal_error_gain_per_m"])
    ...
    def action_bias(self, raw):
        separation = local_vector(raw[..., 0:3] - raw[..., 4:7], raw)
        return torch.cat((self.error_gain * separation[..., :2],
                          torch.zeros_like(separation[..., 2:3])), dim=-1)

    def encode_actions(self, native_actions, chunk_start_context):
        raw = chunk_start_context["raw_current"][:, None, :]
        residual = local_vector(native_actions[..., :3], raw) + self.action_bias(raw)
        return torch.cat((residual, native_actions[..., 3:4]), dim=-1)

    def decode_actions(self, encoded_actions, chunk_start_context):
        raw = chunk_start_context["raw_current"][:, None, :]
        local_command = encoded_actions[..., :3] - self.action_bias(raw)
        return torch.cat((world_vector(local_command, raw), encoded_actions[..., 3:4]), dim=-1)


def build_design(common_spec, config):
    return HorizontalErrorResidualDesign(common_spec, config)
