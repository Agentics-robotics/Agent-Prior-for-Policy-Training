...
class JointRelationalDiffusion(CandidateDesign):
    def build_modules(self, common_dp_factory):
        self.edge_encoder = torch.nn.Sequential(
            torch.nn.Linear(6, 64), torch.nn.SiLU(),
            torch.nn.Linear(64, 16), torch.nn.SiLU())
        self.dp = common_dp_factory(94, diffusion_action_dim=10)
    ...
    def condition(self, causal_history, causal_context):
        ...
        edges = torch.stack((hand_handle_edge, handle_goal_edge, hand_goal_edge), dim=-2)
        learned_edges = self.edge_encoder(edges).flatten(start_dim=-2)
        raw_edges = edges.flatten(start_dim=-2)
        ...
        return torch.cat((raw_edges, learned_edges, side), dim=-1)
...
def build_design(common_spec, config):
    return JointRelationalDiffusion(common_spec, config)
