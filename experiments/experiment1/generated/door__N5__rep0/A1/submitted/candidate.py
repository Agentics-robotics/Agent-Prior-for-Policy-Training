...
class CabinetChart(CandidateDesign):
    ...
    def condition(self, causal_history, causal_context):
        ...
        return torch.cat((
            ...
            h / self.world_scale, g / self.world_scale, o / self.world_scale,
            torch.stack((s, c), dim=-1),
            length(rel) / self.contact_scale,
            length(rel[..., 0:2]) / self.contact_scale,
            length(g - o) / self.world_scale,
            (raw[..., 3:4] - raw[..., 21:22]) / 0.1,
            local_vector(ph - po, s, c) / self.contact_scale,
            local_vector(g - po, s, c) / self.world_scale,
...
def build_design(common_spec, config):
    return CabinetChart(common_spec, config)
