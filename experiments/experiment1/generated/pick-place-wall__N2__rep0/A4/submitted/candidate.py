...
class LiftEvidenceChart(CandidateDesign):
    def fit_support(self, support_view, common_spec):
        samples = []
        for episode in support_view:
            raw = np.asarray(episode['obs'], dtype=np.float32)[:-1]
            commands = np.asarray(episode['actions'], dtype=np.float32)
            selected = raw[commands[:, 3] <= self.design_config['rest_open_command_upper'], 6]
            if len(selected) > 0:
                samples.append(selected)
        if len(samples) == 0:
            samples.append(np.asarray([episode['obs'][0, 6] for episode in support_view], dtype=np.float32))
        heights = np.concatenate(samples).astype(np.float64)
        self.rest_height_m = float(np.median(heights))
        self.rest_sample_count = int(len(heights))

    def deployment_state_dict(self):
        return {'rest_center_z_m': self.rest_height_m, 'support_rest_sample_count': self.rest_sample_count}

    def load_deployment_state_dict(self, state):
        self.rest_height_m = float(state['rest_center_z_m'])
        self.rest_sample_count = int(state['support_rest_sample_count'])
    ...
    def lift_evidence(self, raw):
        fraction = ((raw[..., 6:7] - self.rest_height_m) / self.design_config['lift_confirmation_span_m']).clamp(min=0.0, max=1.0)
        return fraction * fraction * (3.0 - 2.0 * fraction)

    def frame(self, context):
        raw = context['raw_current']
        transport_weight = self.attachment(raw) * self.lift_evidence(raw)
        pickup = raw[:, 4:6] - raw[:, 0:2]
        regularizer = torch.cat([torch.zeros_like(transport_weight), torch.ones_like(transport_weight) * self.design_config['forward_regularizer_m']], dim=-1)
        transport = raw[:, 36:38] - raw[:, 4:6]
        direction = (1.0 - transport_weight) * (pickup + regularizer) + transport_weight * transport
...
def build_design(common_spec, config):
    return LiftEvidenceChart(common_spec, config)
