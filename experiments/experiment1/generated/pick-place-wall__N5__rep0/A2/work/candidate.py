...
class PhaseTransportFrame(CandidateDesign):
    ...
    def frame(self, context):
        ...
        fx = torch.where(length > 1.0e-6, dx / safe, torch.zeros_like(dx))
        fy = torch.where(length > 1.0e-6, dy / safe, torch.ones_like(dy))
    ...
    def condition(self, causal_history, causal_context):
        ...
        fx, fy = self.frame(causal_context)
        ...
        core = torch.cat((lhw, loh, low, hv, ov, grip, q, qp,
                          ...
                          fx * torch.ones_like(grip), fy * torch.ones_like(grip), hf, of, dg), dim=-1)
...
def build_design(common_spec, config):
    return PhaseTransportFrame(common_spec, config)
