...
class ParityTransportJointDiffusion(CandidateDesign):
    ...
    def chart(self, context):
        ...
        dy = direction[:, 1:2]
        parity = torch.where(dy > 0.0, -torch.ones_like(dy), torch.ones_like(dy))
        ...
        sine = torch.where(regular, parity * dy / norm, torch.zeros_like(norm))
        return cosine.unsqueeze(1), sine.unsqueeze(1), parity.unsqueeze(1)

    def to_chart(self, vector, cosine, sine, parity):
        reflected_y = parity * vector[..., 1:2]
        return torch.cat([
            cosine * vector[..., 0:1] + sine * reflected_y,
            -sine * vector[..., 0:1] + cosine * reflected_y,
            vector[..., 2:3]
        ], dim=-1)

    def to_world(self, vector, cosine, sine, parity):
        return torch.cat([
            cosine * vector[..., 0:1] - sine * vector[..., 1:2],
            parity * (sine * vector[..., 0:1] + cosine * vector[..., 1:2]),
            vector[..., 2:3]
        ], dim=-1)

    def condition(self, causal_history, causal_context):
        x = causal_context["raw_history"]
        cosine, sine, parity = self.chart(causal_context)
        hand, peg = x[..., 0:3], x[..., 4:7]
        old_hand, old_peg = x[..., 18:21], x[..., 22:25]
        goal, head = x[..., 36:39], x[..., 39:42]
        reach = self.to_chart(peg - hand, cosine, sine, parity)
        insert = self.to_chart(goal - head, cosine, sine, parity)
        aperture, old_aperture = x[..., 3:4], x[..., 21:22]
        frame_metadata = torch.cat([
            cosine.expand_as(aperture), sine.expand_as(aperture), parity.expand_as(aperture)
        ], dim=-1)
        heights = torch.cat([
            hand[..., 2:3], peg[..., 2:3], head[..., 2:3], goal[..., 2:3]
        ], dim=-1)
        axis_alignment = torch.cat([
            parity * (goal[..., 1:2] - head[..., 1:2]),
            goal[..., 2:3] - head[..., 2:3]
        ], dim=-1)
        return torch.cat([
            self.to_chart(hand - goal, cosine, sine, parity) / self.goal_scale,
            self.to_chart(peg - goal, cosine, sine, parity) / self.goal_scale,
            self.to_chart(head - goal, cosine, sine, parity) / self.goal_scale,
            reach / self.relative_scale,
            self.to_chart(head - peg, cosine, sine, parity) / self.relative_scale,
            self.to_chart(hand - old_hand, cosine, sine, parity) / self.motion_scale,
            self.to_chart(peg - old_peg, cosine, sine, parity) / self.motion_scale,
            self.to_chart(old_peg - old_hand, cosine, sine, parity) / self.relative_scale,
            aperture, old_aperture, aperture - old_aperture,
            x[..., 7:11], x[..., 25:29],
            hand / self.world_scale, goal / self.world_scale,
            heights / self.relative_scale, frame_metadata,
            torch.tanh(reach / self.fine_scale),
            torch.tanh(insert / self.fine_scale),
            axis_alignment / self.relative_scale
        ], dim=-1)

    def encode_actions(self, native_actions, chunk_start_context):
        cosine, sine, parity = self.chart(chunk_start_context)
        return torch.cat([
            self.to_chart(native_actions[..., 0:3], cosine, sine, parity), native_actions[..., 3:4]
        ], dim=-1)

    def decode_actions(self, encoded_actions, chunk_start_context):
        cosine, sine, parity = self.chart(chunk_start_context)
        return torch.cat([
            self.to_world(encoded_actions[..., 0:3], cosine, sine, parity), encoded_actions[..., 3:4]
        ], dim=-1)
    ...
    def diffusion_targets(self, encoded_actions, targets, causal_context):
        ...
        return torch.cat([
            encoded_actions,
            self.to_chart(displacements[..., 0:3], cosine, sine, parity) / self.outcome_scale,
            self.to_chart(displacements[..., 3:6], cosine, sine, parity) / self.outcome_scale
        ], dim=-1)
...
def build_design(common_spec, config):
    return ParityTransportJointDiffusion(common_spec, config)
