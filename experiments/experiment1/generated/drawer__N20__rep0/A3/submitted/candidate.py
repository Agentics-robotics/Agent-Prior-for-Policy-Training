...
class JointRelationalOutcomes(CandidateDesign):
    def build_modules(self, common_dp_factory):
        width = self.design_config['edge_width']
        hidden = self.design_config['edge_hidden']
        self.dp = common_dp_factory(18 + 3 * width + 27, diffusion_action_dim=10)
    ...
    def condition(self, causal_history, causal_context):
        ...
        approach_edge = torch.cat((local_xyz(approach, s, c) / p,
                                   local_xyz(old_approach, s, c) / p,
                                   local_xyz(approach - old_approach, s, c) / d), dim=-1)
        ...
        approach_embedding = self.edge_encoder(torch.cat((approach_edge, one, zero), dim=-1))
        ...
        return torch.cat((approach_edge, travel_edge,
                          approach_embedding, travel_embedding,
                          approach_embedding * travel_embedding, world), dim=-1)
    ...
    def training_targets(self, support_view, window_index):
        ...
        for index, (episode, start) in enumerate(window_index):
            ...
            outcomes[index, :count, :3] = local_numpy(future_offset, s, c) / scale
            outcomes[index, :count, 3:] = local_numpy(handle_displacement, s, c) / scale
        return {'relative_outcomes': outcomes}

    def diffusion_targets(self, encoded_actions, targets, causal_context):
        return torch.cat((encoded_actions, targets['relative_outcomes']), dim=-1)
    ...
    def training_loss(self, output, batch, common_diffusion_state):
        ...
        return self.design_config['outcome_loss_weight'] * auxiliary, {'outcome_epsilon_mse': auxiliary.detach()}


def build_design(common_spec, config):
    return JointRelationalOutcomes(common_spec, config)
