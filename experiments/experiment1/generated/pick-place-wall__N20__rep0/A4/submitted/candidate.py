...
class DualFrameGraspMotion(CandidateDesign):
    def build_modules(self, common_dp_factory):
        width = int(self.design_config["grasp_width"])
        hidden = int(self.design_config["grasp_hidden"])
        self.dp = common_dp_factory(70 + width, diffusion_action_dim=10)
        self.grasp_encoder = torch.nn.Sequential(
            torch.nn.Linear(31, hidden), torch.nn.SiLU(),
    ...
    def condition(self, causal_history, causal_context):
        ...
        path_features = torch.cat([
            ...
            to_local(h - ph, fx, fy) / 0.01,
            to_local(o - po, fx, fy) / 0.01,
            r[..., 3:4], r[..., 21:22],
        ...
        grasp = o - h
        previous_grasp = po - ph
        grasp_features = torch.cat([
            grasp / 0.05, previous_grasp / 0.05,
            (h - ph) / 0.01, (o - po) / 0.01,
            (grasp - previous_grasp) / 0.01,
            r[..., 3:4], r[..., 21:22], r[..., 3:4] - r[..., 21:22],
            canonical_quaternion(r[..., 7:11]),
            canonical_quaternion(r[..., 25:29]),
            h[..., 2:3] / 0.2, o[..., 2:3] / 0.2,
            torch.linalg.vector_norm(grasp[..., :2], dim=-1, keepdim=True) / 0.05,
            torch.linalg.vector_norm(grasp, dim=-1, keepdim=True) / 0.05,
            (h[..., 2:3] - o[..., 2:3]) / 0.05
        ], dim=-1)
        return torch.cat([
            path_features, grasp / 0.05, previous_grasp / 0.05,
            self.grasp_encoder(grasp_features)
        ], dim=-1)
...
def build_design(common_spec, config):
    return DualFrameGraspMotion(common_spec, config)
