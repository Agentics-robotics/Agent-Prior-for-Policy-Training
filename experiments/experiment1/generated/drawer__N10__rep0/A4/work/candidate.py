...
class CompactChainJointDesign(CandidateDesign):
    def build_modules(self, common_dp_factory):
        ...
        self.dp = common_dp_factory(2 * edge + 6 + 26, diffusion_action_dim=8)
    ...
    def condition(self, causal_history, causal_context):
        ...
        # Independent edges of hand -> handle -> goal; hand -> goal is their sum.
        current_edges = torch.stack((cabinet_vector(handle - hand, yaw),
                                      cabinet_vector(goal - handle, yaw)), dim=-2)
        previous_edges = torch.stack((cabinet_vector(previous_handle - previous_hand, yaw),
                                       cabinet_vector(goal - previous_handle, yaw)), dim=-2)
        roles = torch.eye(2, dtype=raw.dtype, device=raw.device).reshape(1, 1, 2, 2)
        roles = roles.expand(raw.shape[0], 2, 2, 2)
        ...
        return torch.cat((tokens.reshape(raw.shape[0], 2, 2 * self.design_config['edge_dim']),
                          current_edges.reshape(raw.shape[0], 2, 6) / distance,
                          world_context), dim=-1)
    ...
    def training_targets(self, support_view, window_index):
        future_motion = np.zeros((len(window_index), 16, 4), dtype=np.float32)
        ...
        for i, (episode_index, t) in enumerate(window_index):
            ...
            future_motion[i, :valid, :3] = cabinet_vector_numpy(hand_delta, yaw) / scale
            # Outward drawer-axis displacement; a target projection, not a controller.
            future_motion[i, :valid, 3] = (yaw[0] * handle_delta[:, 0] - yaw[1] * handle_delta[:, 1]) / scale
        return {'future_motion': future_motion}
    ...
    def training_loss(self, output, batch, common_diffusion_state):
        ...
        auxiliary = (residual.square() * mask.unsqueeze(-1)).sum() / (mask.sum() * 4.0).clamp_min(1.0)
        return self.design_config['joint_loss_weight'] * auxiliary, {'motion_epsilon_mse': auxiliary.detach()}


def build_design(common_spec, config):
    return CompactChainJointDesign(common_spec, config)
