...
def _authorized_gpu(gpu):
    if type(gpu) is not int or gpu not in AUTHORIZED_GPUS:
        raise ValueError("Only physical GPUs 0 through 7 are authorized")
...
def run(root=EXPERIMENT):
    ...
    with locked(root / "locks" / "runner.lock"):
        ...
        if not gpus or len(set(gpus)) != len(gpus) or any(type(gpu) is not int or gpu not in AUTHORIZED_GPUS for gpu in gpus):
