...
class Trainer(BaseTrainer):
    ...
    def update(self):
        ...
        loss=masked_epsilon_loss(epsilon,noise,mask)
...
def _train(rec,implementation,debug_updates,stop_after,device):
    ...
    with torch.random.fork_rng(devices=[]):
        baseline=Policy(cfg['raw_dim'],cfg)
...
class Loaded:
    ...
    def actions(self,history,generator):
        ...
        normalized=torch.as_tensor(self.normalizer.normalize(raw),device=self.device)[None]
        started=time.perf_counter()
