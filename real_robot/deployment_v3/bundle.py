"""Explicit v3 download allowlist, with no training data or credentials."""
...
def build():
    ...
    if manifest.get("api_contract") != "push_training_v3":
        raise ValueError("Expected v3 inference package")
    output = ROOT / "real_robot/exports/push_v3"
    output.mkdir(parents=True, exist_ok=True)
    weights = [
        (WEIGHTS / p["file"], "real_robot/checkpoints/push_v3/" + p["file"])
        for p in manifest["policies"].values()
    ]
    files = [ROOT / "real_robot/__init__.py"]
    files += [
        ROOT / "real_robot/policy_training_v3" / n
    ...
    files += list((ROOT / "real_robot/deployment_v3").glob("*.py"))
    files += [
        ROOT / "real_robot/deployment_v3/README.md",
        PACKAGE / "manifest.json",
        PACKAGE / "TRAINING.md",
    ]
    files += list((PACKAGE / "source").iterdir())
    full = [(p, "push_v3_bundle/" + str(p.relative_to(ROOT))) for p in sorted(files)]
    full += [(p, "push_v3_bundle/" + n) for p, n in weights]
    full += [
        (ROOT / "real_robot/deployment_v3/BUNDLE_README.md", "push_v3_bundle/README.md")
    ]
    archives = {}
    for name, contents in (
        ("push_v3_bundle.tar.gz", full),
        ("push_v3_weights.tar.gz", weights),
    ):
        path = output / name
