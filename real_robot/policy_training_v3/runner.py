"""Developer-owned scheduling and provenance for three independent trainings."""
...
def freeze_executor(cfg, package):
    ...
    paths = list((ROOT / "real_robot/policy_training_v3").glob("*.py")) + [
...
def publish(cfg, revision):
    ...
    for policy_id in cfg["policies"]:
        ...
        if (
            result["optimizer_steps"] != cfg["training"]["updates"]
