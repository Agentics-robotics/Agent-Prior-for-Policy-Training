...
def validate_prepared(prepared, cfg, spec, segments, sources):
    ...
    if (
        arrays["native_action"].shape != (count, 6)
        or not np.isfinite(arrays["native_action"]).all()
    ):
        raise ValueError("Expected finite recorded command concatenation [v,w][K,6]")
    ...
    for k in range(count):
        ...
        action = np.asarray(list(record.get("v") or []) + list(record.get("w") or []))
        if action.shape != (6,) or not np.allclose(
            action, arrays["native_action"][k], rtol=1e-6, atol=1e-7
        ):
            raise ValueError("Native action label differs from recorded v/w command")
...
def prepare(cfg, source, output):
    ...
    manifest = dict(
        schema="real_robot.prepared_cache.v3",
...
def fit(cfg, source, cache, output, policy_id):
    ...
    with torch.no_grad():
        ...
        if predicted.shape != (len(indices), 6) or not torch.isfinite(predicted).all():
            raise ValueError("Prediction must be finite candidate recorded v/w[B,6]")
