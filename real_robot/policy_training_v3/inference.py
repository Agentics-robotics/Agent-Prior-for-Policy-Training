"""Local observation-in/action-out worker; never sends commands to a robot."""
...
class PolicyProcess:
    ...
    act computes candidate actions only; decode returns a command description
...
def serve(cfg, package, output, policy_id):
    ...
    public.configure(None, [], output, assets)
    ...
    for line in sys.stdin:
        ...
        if action is not None and (
            np.asarray(action).shape != (6,) or not np.isfinite(action).all()
        ):
            raise ValueError("Online policy returned invalid candidate recorded v/w")
        answer = {k: v for k, v in result.items() if k != "memory"}
        if request.get("controller_contract") is not None and action is not None:
            answer["decoded"] = module.decode_action(
