...
from real_robot.flip_egg_cut_v1.data import ROOT, CONTRACT, Sources, config
from real_robot.flip_egg_cut_v1.tools import CutTools
from real_robot.flip_egg_cut_v1.transport import Client
...
def freeze_interface(cfg):
    run = ROOT / cfg["run"]
    paths = list((ROOT / "real_robot/flip_egg_cut_v1").glob("*.py")) + [
        Path(__file__).resolve(),
        ROOT / "real_robot/flip_egg_cut_v1/__init__.py",
        ROOT / "real_robot/flip_egg_cut_v1/tools.py",
        ROOT / cfg["prompt"],
...
def run(cfg):
    ...
    message = dict(
        task="Inspect the authorized demonstrations and supplied execution capability. Submit a complete system responsibility map, data groups, decision supervision/target contracts, chosen learned priors and callable policy/executor interfaces. Choose the learning problems, outputs and policy count. Stop before implementation.",
        ...
        heuristic_count="Choose from distinct remaining learning needs; no requested portfolio size or quota of alternatives. Supplied executor groups have no learned policy.",
    )
...
def verify(cfg):
    ...
    for item in manifest["datasets"]:
        ...
        for key in ("component_kind", "execution_contract"):
            if dataset[key] != source_skill[key]:
                raise ValueError("Execution responsibility was rewritten")
    ...
    result = dict(
        ...
        learned_groups=manifest["checks"]["learned_groups"],
        supplied_executor_groups=manifest["checks"]["supplied_executor_groups"],
...
def report(cfg, plan, manifest, result):
    ...
    (report_root / "FLIP_EGG_CUT_V1.md").write_text("\n".join(lines))
...
def main():
    ...
    p.add_argument("--config", default="real_robot/configs/flip_egg_cut_v1.json")
    args = p.parse_args()
