...
def freeze_interface(cfg):
    ...
    paths = list((ROOT / "real_robot").glob("*.py")) + [
        ...
        ROOT / "real_robot/cutting_v6/__init__.py",
        ROOT / "real_robot/cutting_v6/tools.py",
        ROOT / cfg["prompt"],
...
def report(cfg, plan, manifest, result):
    ...
    (report_root / "PUSH_CUT_V6.md").write_text("\n".join(lines))
...
def main():
    ...
    p.add_argument("--config", default="real_robot/configs/push_cut_v6.json")
    args = p.parse_args()
