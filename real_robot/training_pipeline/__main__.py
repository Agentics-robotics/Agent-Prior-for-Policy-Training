...
def main():
    ...
    parser.add_argument("command", choices=["run", "preflight", "execute", "worker", "status", "publish"])
    parser.add_argument("--config", default="real_robot/configs/push_training_v5.json")
    ...
    if args.command == "worker":
        ...
        if not args.device_isolated and not cfg.get("test_cpu"):
            from appl.gpu import launch

            return launch(args.gpu, sys.argv[1:], module="real_robot.training_pipeline")
        nodes = {
            p.name
            for p in Path("/dev").glob("nvidia*")
            if p.name.removeprefix("nvidia").isdigit()
        }
        if not cfg.get("test_cpu") and nodes != {"nvidia" + os.environ["APPL_GPU_MINOR"]}:
    ...
    if args.command in {"run", "preflight"}:
        from .workflow import run, preflight
        value = preflight(cfg) if args.command == "preflight" else run(cfg)
