...
class ImplementationFailure(RuntimeError):
    def __init__(self, diagnostics):
        super().__init__("API implementation failed during isolated execution")
...
def freeze_executor(cfg, package):
    ...
    paths = list((ROOT / "real_robot/training_pipeline").glob("*.py")) + [
...
def start_job(cfg, package, operation, output, gpu, policy_id=None):
    ...
    if not cfg.get("test_cpu"):
        lock_path = ROOT / "real_robot/runs/.training_pipeline_locks" / f"gpu_{gpu}.lock"
        lock_path.parent.mkdir(parents=True, exist_ok=True)
        allocation_lock = lock_path.open("a+")
        fcntl.flock(allocation_lock, fcntl.LOCK_EX)
    try:
        device = None if cfg.get("test_cpu") else identity(gpu)
        occupancy = "CPU infrastructure fixture" if cfg.get("test_cpu") else subprocess.check_output(
            [
                "nvidia-smi",
        ...
        return dict(
            ...
            policy_id=policy_id,
            timeout_seconds=cfg["max_job_seconds"],
            allocation_lock=allocation_lock,
        )

    except BaseException:
        if allocation_lock is not None:
            allocation_lock.close()
        raise
...
def finish_job(job):
    timed_out = False
    try:
        code = job["process"].wait(timeout=job["timeout_seconds"])
    ...
    job["stdout"].close()
    job["stderr"].close()
    if job["allocation_lock"] is not None:
        job["allocation_lock"].close()
    result = dict(
        returncode=code,
...
def execute(cfg, revision):
    ...
    submission = read(package / "submission.json")
    if submission["readiness"]["decision"] != "train":
        raise ValueError("API declared this package blocked for training")
    atomic(
    ...
    if (cache / "result.json").exists():
        ...
        if digest(cache / "result.json") != submission["preparation_hash"]:
            raise ValueError("Readiness assessment refers to a different cache")
        ...
        if prepared["returncode"]:
            ...
            raise ImplementationFailure(prepared)
    atomic(
        root / "execution_status.json",
        dict(status="training", revision=revision, policies=list(read(package / "source/package.json")["policies"])),
    )
    jobs = []
    for i, policy_id in enumerate(read(package / "source/package.json")["policies"]):
        gpu = cfg["devices"][i % len(cfg["devices"])]
        output = package / "training" / policy_id
        if (output / "result.json").exists():
            continue
        jobs.append(finish_job(start_job(cfg, package, "train", output, gpu, policy_id)))
    results = jobs
    if any(r["returncode"] for r in results):
        ...
        raise ImplementationFailure(results)
...
def publish(cfg, revision):
    ...
    policies = []
    for policy_id in read(package / "source/package.json")["policies"]:
        result = read(package / "training" / policy_id / "result.json")
        if (
            not 1 <= result["optimizer_steps"] <= read(package / "source/package.json")["policies"][policy_id]["updates"]
