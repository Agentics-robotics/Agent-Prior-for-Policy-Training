...
def build():
    output = ROOT / "real_robot/exports/pipeline_v1"
    ...
    files.update((ROOT / "real_robot/deployment_pipeline").glob("*.py"))
    files.add(ROOT / "real_robot/deployment_pipeline/README.md")
    weights,assets = [],[]
    ...
    full = [(p,"pipeline_v1_bundle/"+str(p.relative_to(ROOT))) for p in sorted(files)]
    full += [(p,"pipeline_v1_bundle/"+n) for p,n in weights+assets]
    archives = {}
    for name,contents in (("pipeline_v1_weights.tar.gz",weights),
                          ("pipeline_v1_sam_assets.tar.gz",assets),
                          ("pipeline_v1_bundle.tar.gz",full)):
        path = output / name
        archive(path,contents)
        archives[name] = dict(bytes=path.stat().st_size,sha256=digest(path))
    record = dict(schema="real_robot.pipeline_download.v1",archives=archives,
