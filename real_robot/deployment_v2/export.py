...
def export(training_package, package=PACKAGE, weights=WEIGHTS):
    ...
    lines += [
        "",
        f"The shared prepared dataset contains {prepared['num_examples']} examples from {sum(v > 0 for v in counts.values())}/{len(counts)} source cuts. The following counts are actual included supervision, not merely proposed segment support.",
        "",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--training-package",
        default=str(ROOT / "real_robot/runs/push_letters/training_v2/package_00"),
    )
