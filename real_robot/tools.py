...
from .data import CONTRACT, ROOT, Sources
...
SEGMENT = obj(
    dict(
        **RANGE,
        ...
        supervised_stop=dict(type="integer", minimum=1),
...
class CutTools:
    ...
    def schemas(self):
        ...
        return [
            ...
            tool(
                "write_plan",
                "Write an API-authored plan. skills entries are dataset groups, not one-policy-per-group. Each semantic segment declares materialized [start,stop), nested [supervised_start,supervised_stop), conditions/label rules, exclusions, merge/split checks and inspected evidence. Boundary evidence may include neighboring source rows; condition evidence must be within the materialized segment. Each heuristic has a unique heuristic_id and policy_id with its caller contract. One or multiple independent policies per group; count is your decision. No code.",
                dict(plan=PLAN),
            ),
    ...
    def validate(self, plan):
        ...
        for skill in plan["skills"]:
            ...
            for s in skill["segments"]:
                ...
                if not lo <= slo < shi <= hi:
                    raise ValueError(
                        "Supervised range must be inside its materialized segment"
                    )
                ...
                if not {slo, shi - 1}.issubset(boundaries):
                    raise ValueError(
                        "Boundary evidence must include first/last supervised rows"
                    )
                if not (evidence | boundaries | {slo, shi - 1}).issubset(
                ...
                if not {slo, shi - 1}.issubset(visual):
                    raise ValueError(
                        "Read_images must inspect first/last supervised rows in a camera view"
                    )
    ...
    def markdown(skill):
        ...
        for s in skill["segments"]:
            lines += [
                f"### {s['segment_id']}",
                "",
                f"Source: `{s['trajectory_id']}` [{s['start']}, {s['stop']}). Supervised interval: [{s['supervised_start']}, {s['supervised_stop']}).",
                "",
            ]
    ...
    def publish(self, version, plan, checks):
        ...
        for skill in plan["skills"]:
            ...
            dataset = dict(
                schema="real_robot.cut_dataset.v2",
                plan_hash=version,
