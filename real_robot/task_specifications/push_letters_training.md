...
## Implementation task facts

Only the two authorized recordings above are available as demonstrations. This
is a small dataset. There are no additional verified labels or calibration
measurements. The previous API-authored cut/prior package is supplied separately
as design context. The current stage implements preprocessing, callable policies
and training; its stage boundary is defined by the generic workflow interface.
The real robot is not connected to this training process.
