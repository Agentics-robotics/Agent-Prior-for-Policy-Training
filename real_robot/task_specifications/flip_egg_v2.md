...
The user requests an independent API cut/prior design for `flip_egg`, and wants
to understand the decomposition and each proposed model's inputs and outputs.
...
## User-supplied physical task emphasis

The user identifies getting the spatula under and lifting the egg, and turning
the egg over, as the central interactions to accomplish. The system may also
need to acquire the tool and bring it near the egg. Inspect the demonstrations
to establish their actual geometry, motion, prerequisites and outcomes, and
choose how these responsibilities are learned, shared, or supplied by execution.
...
Produce API-owned cuts, freely regrouped datasets, supervision definitions,
priors, model/policy inventories, preprocessing requirements, executor adapters
and High-level Agent calling documents. There is no prescribed cut count,
policy count, output dimensionality, or quota of alternative heuristics.
Stop before implementation, training, policy evaluation or robot execution.
The concurrent Push run and all original recordings remain separate.
