"""Read-only scientific audit and timeline of the exact API cut_v5 submission."""
...
def main():
    ...
    for tid, ep in source.episodes.items():
        ...
        for raw in ep["arrays"]["action_json"]:
            action = json.loads(str(raw))
            command = action.get("dq")
            values = np.asarray(command, dtype=np.float64)
            valid.append(values.shape == (7,) and bool(np.isfinite(values).all()))
            v = np.asarray(action.get("v"), dtype=np.float64)
            w = np.asarray(action.get("w"), dtype=np.float64)
            valid_twist.append(v.shape == (3,) and w.shape == (3,)
                               and bool(np.isfinite(v).all() and np.isfinite(w).all()))
        command_valid[tid] = np.asarray(valid)
        twist_valid[tid] = np.asarray(valid_twist)
    ...
    for group_index, skill in enumerate(plan["skills"]):
        ...
        for segment in skill["segments"]:
            ...
            segment_rows[-1]["finite_v_w_candidates"] = int((mask & twist_valid[tid]).sum())
            ax = episode_axes[tid]
        ...
        group_rows[-1]["finite_v_w_supervised_unique"] = sum(int((v & twist_valid[tid]).sum()) for tid, v in group_supervised.items())
    ...
    for tid in assigned:
        ...
        coverage[tid] = actual
        coverage[tid]["finite_v_w_supervised_unique"] = int(((sup > 0) & twist_valid[tid]).sum())

    for tid, ax in episode_axes.items():
        for group_index, skill in enumerate(plan["skills"]):
            boundaries = sorted({x[k] for x in skill["segments"] if x["trajectory_id"] == tid
                                 for k in ("supervised_start", "supervised_stop")})
            ax.vlines(boundaries, group_index - 0.2, group_index + 0.2,
                      colors="white", linewidth=0.7, alpha=0.9)
        ax.set_title(tid)
    ...
    fig.suptitle("API cut_v5: subtask datasets and overlap\nLight: retained interval; dark: action-supervision candidates before input preprocessing", fontsize=13)
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    figure = ROOT / "real_robot/reports/PUSH_CUT_V5_TIMELINE.png"
    fig.savefig(figure, dpi=150)
    plt.close(fig)

    previous_freeze = read(ROOT / "real_robot/runs/push_letters/cut_v4/interface_freeze.json")
    for name, expected in previous_freeze["files"].items():
        assert digest(ROOT / name) == expected, name
    previous_manifest = read(ROOT / "real_robot/data/push_letters/cut_v4/manifest.json")
    for name, expected in previous_manifest["files"].items():
        assert digest(ROOT / "real_robot/data/push_letters/cut_v4" / name) == expected, name
    ...
    review["action_count_definitions"] = dict(
        finite_command="Seven finite original action.dq values; auxiliary follower check in the cut_v5 design",
        finite_v_w="Three finite original action.v plus three finite action.w values; proposed primary cut_v5 behavior targets",
        eligibility="Field finiteness only; controller semantics, derived geometry and phase eligibility remain unverified",
    )
