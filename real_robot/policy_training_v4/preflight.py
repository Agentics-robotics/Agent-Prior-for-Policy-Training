...
def main():
    cfg = configuration(ROOT / "real_robot/configs/push_training_v4.json")
    ...
    old = {}
    for cut in ("cut_v3", "cut_v4", "cut_v5", "cut_v6"):
    ...
    assert {g["skill_id"] for g in plan["skills"] if g["component_kind"] == "learned_policy"} == set(DATASETS.values())
    for path in (ROOT / "real_robot/policy_training_v4").glob("*.py"):
    ...
    try:
        ...
        atomic(setup / "preflight.json", dict(
            ...
            contract_tests="See setup/interface_tests.json for sparse-target provenance checks",
