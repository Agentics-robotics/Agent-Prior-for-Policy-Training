...
def specification(cfg, source):
    ...
    segments = [dict(s, dataset_group=g["skill_id"])
                for g in plan["skills"] for s in g["segments"]]
    spec["segment_dataset_groups"] = [s["dataset_group"] for s in segments]
...
def validate_prepared(prepared, cfg, spec, segments, sources):
    """Validate provenance and per-policy sampling, without scientific filtering."""
    ...
    if not required <= arrays.keys():
        raise ValueError(
            "Missing decision provenance arrays: " + str(required - arrays.keys())
        )
    count = metadata["num_examples"]
    if type(count) is not int or count < 1:
        raise ValueError("No eligible examples returned")
    if not {"coverage", "anchor_audit", "label_provenance"} <= metadata.keys():
        raise ValueError("Account for all sparse anchors and label provenance")
    total = 0
    for name, array in arrays.items():
        ...
        if array.dtype.kind in "fc" and not np.isfinite(array).all():
            raise ValueError("Prepared model arrays must be finite with explicit missingness masks: " + name)
    ...
    if (weight.shape != (count, len(cfg["policies"]))
            or not np.isfinite(weight).all() or np.any(weight < 0)
            or np.any(weight.sum(axis=1) <= 0) or np.any(weight.sum(axis=0) <= 0)):
        raise ValueError("policy_weight[K,P] must be finite nonnegative with positive row/column sums")
    counts = {s["segment_id"]: 0 for s in segments}
    policy_counts = {p: dict.fromkeys(counts, 0) for p in cfg["policies"]}
    seen = set()
    for k in range(count):
        ...
        provenance = (ep_index, index, ordinal)
        if provenance in seen:
            raise ValueError("Duplicate identical example provenance; use sampling weights")
        seen.add(provenance)
        if (
            segment["trajectory_id"] != tid
            or segment["supervision_kind"] != "sparse"
            or index not in segment["decision_indices"]
        ...
        if not segment["start"] <= arrays["observation_start"][k] < arrays["observation_stop"][k] <= index + 1:
            raise ValueError("Observation window must be causal and within materialized segment")
        if not segment["start"] <= arrays["target_start"][k] <= index < arrays["target_stop"][k] <= segment["stop"]:
            raise ValueError("Target evidence must remain inside materialized segment and include its decision anchor")
        counts[segment["segment_id"]] += 1
        for j, policy_id in enumerate(cfg["policies"]):
            if weight[k, j] > 0:
                if segment["dataset_group"] != cfg["policy_datasets"][policy_id]:
                    raise ValueError("Policy example belongs to another frozen dataset group")
                policy_counts[policy_id][segment["segment_id"]] += 1
    expected = {(s["trajectory_id"], s["segment_id"], i)
                for s in segments if s["supervision_kind"] == "sparse"
                for i in s["decision_indices"]}
    audited = set()
    retained = set()
    for entry in metadata["anchor_audit"]:
        key = (entry["trajectory_id"], entry["segment_id"], entry["source_index"])
        if key not in expected or key in audited or entry["status"] not in {"retained", "excluded"} or not entry["reason"]:
            raise ValueError("anchor_audit must enumerate each frozen decision once with status and reason")
        audited.add(key)
        if entry["status"] == "retained":
            retained.add(key)
    actual = {(spec["trajectory_ids"][e], segments[s]["segment_id"], i) for e, i, s in seen}
    if audited != expected or retained != actual:
        raise ValueError("Anchor audit differs from frozen decisions or actual retained rows")
    return dict(
        num_examples=count, segment_examples=counts,
        ...
        cache_bytes=total,
        frozen_sparse_anchors=len(expected), excluded_sparse_anchors=len(expected - retained),
    )
...
def prepare(cfg, source, output):
    ...
    for policy_id in cfg["policies"]:
        ...
        col = cfg["policies"].index(policy_id)
        indices = np.flatnonzero(arrays["policy_weight"][:, col] > 0)[:2]
        model = module.build_model(policy_id, model_spec).to("cuda:0")
    ...
    manifest = dict(
        schema="real_robot.prepared_cache.v4",
...
def fit(cfg, source, cache, output, policy_id):
    ...
    if updates > training["max_updates"]:
        raise ValueError("API-declared update count exceeds execution bound")
    request = dict(
        policy_id=policy_id,
        source_hashes=hashes,
    ...
    policy_column = cfg["policies"].index(policy_id)
    probabilities = np.asarray(arrays["policy_weight"][:, policy_column], dtype=np.float64).copy()
    ...
    for step in range(1, updates + 1):
        warmup = params["warmup_updates"]
        factor = step / warmup if step <= warmup else 0.5 * (1 + math.cos(math.pi * (step - warmup - 1) / (updates - warmup)))
    ...
    with torch.no_grad():
        ...
        if predicted.shape != (len(indices), params["prediction_dimension"]) or not torch.isfinite(predicted).all():
            raise ValueError("Prediction must be finite [B,declared prediction_dimension]")
    ...
    result = dict(
        ...
        dataset_group=cfg["policy_datasets"][policy_id],
