...
def _checked_result(path):
    ...
    stage=result['identity']['stage']
    assert stage in COUNTS
    assert len(result['records'])==sum(COUNTS[stage].values())
    assert len({r['episode_id'] for r in result['records']})==len(result['records'])
    ...
    for split, count in COUNTS[stage].items():
        ...
        assert result['metrics'][split]==metrics(rows), f'Cached metrics changed: {path} {split}'
    assert result['ood_success_rate']==sum(result['metrics'][s]['success_rate'] for s in ('C','E'))/2
    return result


def _training_artifacts(run):
    """Validate formal training identity before executing current policy code."""
    directory=R3/'checkpoints'/run['run_id']
    training=read_json(directory/'complete.json')
    assert training['step']==STEPS and training['debug'] is False
    assert training['run_id']==run['run_id']
    checkpoint=directory/'checkpoints'/'step_020000.pt'
    assert sha256(checkpoint)==training['checkpoints'][str(STEPS)]['sha256']
    assert (ROOT/training['checkpoints'][str(STEPS)]['path']).resolve()==checkpoint.resolve()
    if run.get('train_complete_sha256'):
        assert sha256(directory/'complete.json')==run['train_complete_sha256']
    implementation=R3/'configs'/run['task']/(run['candidate_id']+'.json')
    config=read_json(implementation)
    assert object_hash(config)==training['implementation_hash']
    assert object_hash(read_json(directory/'config.json'))==training['config_hash']
    if run['candidate_id']=='B0':
        proposal=R3/'ROUND3_SPEC.txt'
    elif run['candidate_id']=='P4':
        proposal=R3/'design_records'/run['task']/'revision.json'
        assert _feedback_decision(run['task'])=='P4' and run['train_n'] in (5,20)
    else:
        proposal=R3/'design_records'/run['task']/'proposal.json'
        frozen=read_json(proposal.with_name('initial_freeze.json'))
        assert frozen['proposal_sha256']==sha256(proposal)
    assert config['proposal_sha256']==sha256(proposal), 'Implementation proposal identity mismatch'
    identity=read_json(directory/'identity.json')
    assert object_hash(identity['source_hashes'])==training['code_hash']
    for source,digest in identity['source_hashes'].items():
        assert sha256(ROOT/source)==digest, f'Inference source changed after training: {source}'
    assert read_json(R3/'data'/run['task']/'manifest.json').get('complete') is True
...
def worker(run_id, stage, device="cuda"):
    ...
    with lock('run-'+run_id,blocking=False):
        return _worker(run_id,stage,device)


def _worker(run_id, stage, device="cuda"):
    ...
    checkpoint,training_complete = _training_artifacts(run)
    ...
    if complete_path.exists():
        ...
        update_run(run_id,status='completed' if stage=='test' else 'pending',pid=None,worker_pid=None,
            **{stage+'_status':'completed',stage+'_result_path':str(complete_path.relative_to(ROOT)),
               stage+'_result_hash':sha256(complete_path)})
        return old
    for split in SPLITS:
        assert len(planned[split]) == COUNTS[stage][split]
    update_run(run_id,status='running',pid=os.getpid(),worker_pid=os.getpid(), **{stage + "_status": "running"})
...
def freeze_test_gate():
    ...
    paths = [R3 / "selection.json", Path(__file__), R3/'design_records'/'feedback_frame_rule.json']
    for task in TASKS:
        ...
        paths.extend([proposal, revision, proposal.with_name('initial_freeze.json'),
                      R3 / "data" / task / "manifest.json",R3/'protocol_and_task_manifests'/(task+'.json')])
        paths.extend(p for name in ('initial_feasibility_revision.json','feedback_feasibility_revision.json')
                     if (p:=proposal.with_name(name)).exists())
        feedback=proposal.parent/'feedback'/'bundle.json'
        assert feedback.exists(), f'Feedback packet must precede its decision: {task}'
        packet=read_json(feedback)
        assert packet['contains_locked_test'] is False and packet['frame_count']<=8
        for name,digest in packet['bound_files'].items():
            assert sha256(ROOT/name)==digest, f'Feedback evidence changed: {name}'
        paths.append(feedback)
        paths.extend(ROOT/p for p in packet['bound_files'])
    ...
    for run in runs:
        if run.get("status") == "invalid":
            continue
        rid = run["run_id"]
        _training_artifacts(run)
        training_dir=R3/'checkpoints'/rid
        paths.extend([training_dir/'complete.json',training_dir/'identity.json',training_dir/'config.json'])
        paths.extend(ROOT/p for p in read_json(training_dir/'identity.json')['source_hashes'])
    ...
    paths.extend([Path(__file__).with_name('environment.py'),Path(__file__).with_name('tasks.py')])
