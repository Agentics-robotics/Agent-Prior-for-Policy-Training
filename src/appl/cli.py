...
def main():
    ...
    parser.add_argument('command',choices=['audit','check','diagnose-dp','design','train','evaluate','report','viewer','worker'])
    ...
    if args.command in ('diagnose-dp','evaluate','worker') and not args.device_isolated:
        from .gpu import launch
        default=cfg['devices']['render'] if args.command=='evaluate' or (args.command=='diagnose-dp' and args.stage=='D0') else cfg['devices']['train']
    ...
    if args.command=='audit':
        ...
        if args.stage=='reconstruct':
            ...
            reconstruct(cfg)
        elif args.stage=='capability':
            from .design import capability
            capability(cfg)
        elif args.stage in ('library','D0'):
            from .library import design
            design(cfg)
        else:raise ValueError('Unknown API design stage')
    elif args.command=='evaluate':
        if args.stage=='capability':
            from .evaluate import capability
            value=capability(cfg)
        else:
            from .formal import evaluate
            value=evaluate(cfg)
        print(json.dumps(value,indent=2))
    elif args.command=='train':
        from .formal import train
        value=train(cfg)
        print(json.dumps(dict(training_jobs=len(value['jobs'])),indent=2))
