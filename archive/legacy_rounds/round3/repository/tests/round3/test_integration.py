...
def test_eight_authorized_gpu_slots_are_scheduled_concurrently(monkeypatch):
    devices = [0, 1, 2, 3]
    assert list(cli.GPUS) == devices
    assert cli.WORKERS_PER_GPU == 2
    expected_slots = [(gpu, slot) for slot in (0, 1) for gpu in devices]
    barrier = Barrier(len(expected_slots), timeout=5)
    def queue(gpu, stages, slot):
        assert stages == ["train"]
        barrier.wait()
        return gpu, slot
    monkeypatch.setattr(cli, "lock", unlocked)
    ...
    assert cli.schedule(["train"]) == expected_slots
