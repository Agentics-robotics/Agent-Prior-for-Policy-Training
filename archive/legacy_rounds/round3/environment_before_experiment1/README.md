...
`dev` 分支新增 GPT-6 API、统一 `Env` 抽象、MetaWorld/ManiSkill 适配器及策略 rollout 命令。原先验提取流程和历史实验源码保持不变。使用方式、空机器人接口和验证限制见 [接口文档](INTERFACES.md)，新增开发规则见 [agent.md](agent.md)。

**当前工作为 Round 4：ManiSkill 3 + PartNet Mobility 跨柜体抽屉实验，处于准备与可行性验证阶段。** 当前没有 Round 4 策略训练、学习策略评测或先验收益结果。执行要求见 [ROUND4_SPEC.md](ROUND4_SPEC.md)，已完成事项与待办见 [ROUND4_REPORT.md](ROUND4_REPORT.md)。

当前阻碍：官方柜体资产端点连接超时，本机无缓存；已完成固定底盘 Fetch 控制实测及 20 项通用训练 CPU 检查，真实柜体示范与训练待资产恢复。详见[资产调查](round4/audits/training_asset_availability.md)。

## Round 4：准备与可行性验证（2026-09-09）

本轮研究已知模拟器几何条件下的跨柜体控制泛化。先完成候选训练柜体上的控制器验证、成功专家示范收集及带视频的动作重放，再冻结按完整柜体 model ID 隔离的训练/开发/测试划分、共同观测和嵌套示范子集，随后开展单 seed 试验与三个训练 seed 的主实验。准备工作和环境导入检查不代表上述可行性门槛或正式实验已经完成。

ManiSkill 使用同一个 Pixi 工作区中的独立 `round4` 环境，命令为 `pixi run -e round4 ...`；MetaWorld 继续使用默认环境 `pixi run ...`。所有 Python 和项目命令均通过 Pixi。默认 MetaWorld 环境的 98 个锁定包记录与迁移前归档完全一致，实际导入确认 PyTorch 2.7.1、NumPy 2.2.6、Gymnasium 1.2.0、MuJoCo 3.3.0 及固定 commit 的 MetaWorld 3.1.1 保持不变，证据见 [环境保留核对](round4/audits/metaworld_environment_preservation.json)。最新授权仅允许物理 GPU **0–3**；Round 4 并发数量按实际显存和模拟器限制确定，并保留无关进程。

按用户授权，已删除 Round 3 的 **418 个权重文件，回收 30.06 GiB**。保留 drawer N10 的 **B0/P1/P2/P3** 四模型，每个模型包含 5k/10k/20k 检查点和 latest，共 16 个文件，均通过 CPU 加载验证；所有数据、配置、身份、日志、评测、报告及源码证据保留。精确文件路径、SHA256 和验证结果见 [删除与保留回执](round4/audits/round3_checkpoint_retention.json) 及 [删除前清单](round4/audits/round3_checkpoint_inventory_before_cleanup.json)。

下方 Round 1/2/3 内容保留历史交付时的记录和命令。Round 3 最终验收描述的是删除前的完整交付；按本次存储保留决策，完整检查点验收与所有模型的权重复评现已无法原样重跑。已删除权重不重新排队训练补回，历史结果与审计文件也不改写。迁移前的环境和首页原文见 [归档清单](round4/provenance/pre_round4_20260909T111946Z/manifest.json)。
