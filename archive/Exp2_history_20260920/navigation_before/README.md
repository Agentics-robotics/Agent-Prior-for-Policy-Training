...
**Exp2_new OOD 已全部完成并停止（2026-09-20）：** 五任务各15个OOD，APPL **30/75（40%）**，同初态修正DP **5/75**，SinglePrior **10/75**，0未决结果。未新增ID，仍为8/10。追加500 SGD范围内用量费用估算334.56 USD；所有进程退出，原模型/实验保留。[最终结果](runs/exp2/Exp2_new/ood_resource_resume_20260920/REPORT.md)、[费用和执行分析](runs/exp2/Exp2_new/ood_resource_resume_20260920/ANALYSIS.md)、[完成核验](runs/exp2/Exp2_new/ood_resource_resume_20260920/completion.json)、[全部75个视频](runs/exp2/Exp2_new/ood_resource_resume_20260920/replays.html)。以下为此前执行历史。

2026-09-20 **Exp2_new OOD 续跑**：用户追加 500 SGD，已启动剩余 68 项，只补齐五任务各 15 个 OOD 后停止，ID 不再调度。原 7 个完整 OOD 直接复用，原预算中断项保留并从同初态重测。[当前报告](runs/exp2/Exp2_new/ood_continuation_20260920/REPORT.md)、[进度](runs/exp2/Exp2_new/ood_continuation_20260920/results.json)、[视频](runs/exp2/Exp2_new/ood_continuation_20260920/replays.html)。

2026-09-20 **[Exp2_new](experiments/exp2/EXP2_NEW.md) 已因预算停止并完成核验**：复用 Astra/xhigh 的 36 个 policy，只重新部署 APPL；150 回合计划中完成 17 个（ID 8/10、OOD 4/7），另有 1 个中断、132 个未启动。用量费用估算 77.57 USD，下次请求预留超出本轮 100 SGD 对应的保守 78 USD 上限；没有服务端拒绝或自动重试。两个修正 baseline 引用同初态已有结果。见[报告](runs/exp2/Exp2_new/REPORT.md)、[配对分析与费用](runs/exp2/Exp2_new/ANALYSIS.md)、[视频](runs/exp2/Exp2_new/replays.html)及[完成核验](runs/exp2/Exp2_new/completion.json)。全部进程已退出，原实验与模型保留。
