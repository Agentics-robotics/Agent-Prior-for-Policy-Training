...
def normalizer(data, cfg):
    ...
    if cfg.get('action_coordinates','absolute')!='absolute':
    ...
    scaling=cfg.get('observation_normalization','standard')
    if scaling=='standard':
        mean=obs.mean(0);std=np.maximum(obs.std(0),cfg['observation_std_floor'])
    elif scaling=='limits':
        lo,hi=obs.min(0),obs.max(0)
        constant=hi-lo<1e-4
        mean=(hi+lo)/2;std=(hi-lo)/2
        mean[constant]=lo[constant];std[constant]=1.
    else:raise ValueError('Unknown observation normalization')
