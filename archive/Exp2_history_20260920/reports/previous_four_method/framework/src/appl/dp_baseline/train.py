...
def sample(model,module,raw,spec,generator):
    config=spec['training'];kind=config.get('sampler','ddim')
    if config.get('action_coordinates','absolute')!='absolute':
        raise ValueError('Action-coordinate experiments are archived in archive/Exp2_M0DP')
    if kind not in ('ddim','ddpm'):raise ValueError('Unknown diffusion sampler')
    s=scheduler(config,DDIMScheduler if kind=='ddim' else DDPMScheduler)
    ...
    for t in s.timesteps:
        ...
        kwargs=dict(eta=0) if kind=='ddim' else dict(generator=generator)
        value=s.step(predicted,t,value,**kwargs).prev_sample
...
def fit(cfg,output,*,ids=None,segments=None,candidate=None,updates=None,seed=0,fixed_batch=False,lockdown=None,stop_after_checkpoint=None):
    ...
    for step in range(start_step+1,config['updates']+1):
        ...
        if schedule=='cosine':
            ...
            if not 0<=warmup<config['updates']:raise ValueError('Invalid warmup budget')
            factor=step/warmup if step<=warmup else .5*(1+math.cos(math.pi*(step-warmup)/(config['updates']-warmup)))
            for group in optimizer.param_groups:group['lr']=config['learning_rate']*factor
        elif schedule!='constant':raise ValueError('Unknown learning-rate schedule')
        ...
        if step%100==0 or step==1:
            progress=dict(step=step,loss_mean=float(np.mean(losses)),learning_rate=optimizer.param_groups[0]['lr'],elapsed_seconds=elapsed_before+time.monotonic()-started,
...
class LoadedPolicy:
    def __init__(self,checkpoint,candidate=None,seed=0):
        ...
        if c['spec']['training'].get('action_coordinates','absolute')!='absolute':
            raise ValueError('Action-coordinate experiments are archived in archive/Exp2_M0DP')
